<?php include(APPPATH . 'Views/includes/header.php'); ?>


<!--==================================================-->
<!-- start	softech slider Area -->
<!--==================================================-->
<div class="breatcumb-area d-flex align-items-center">
	<div class="container">
		<div class="row">
			<div class="col-lg-12">
				<div class="breatcumb-content text-center">
					<div class="breatcumb-title">
						<h2>Company</h2>
					</div>
					<div class="breatcumb-content-text">
						<ul>
							<!-- <li><a href="#">home</a> <i class="fas fa-angle-right"></i> <span>Company</span></li> -->
						</ul>
					</div>	
				</div>
			</div>
		</div>
	</div>
</div>
<!--==================================================-->
<!-- End	softech slider Area -->
<!--==================================================-->

<!--==================================================-->
<!-- Start dreamit about Area -->
<!--==================================================-->
<div class="about-area pt-90 pb-95">
	<div class="container">
		<div class="row">
			<div class="col-lg-6 col-md-6 col-sm-12">
				<div class="dreamit-about-thumb">
					<img src="assets/images/about/about1.png" alt="">
				</div>
			</div>	
			<div class="col-lg-6 col-md-6 col-sm-12">
				<div class="row">
					<div class="col-lg-12">
						<div class="dreamit-section-title text-left">
							<h5>About</h5>
							<h2>1-2 years Experience In</h2>
							<h2>The Handyman </h2>
							<div class="em_bar_bg text-left"></div>
							<p>Crafting Excellence in Handyman & Plumbing Services for Two Decades, Building Trust Through Masterful Artistry and Unparalleled Customer Satisfaction in Every Home Service Endeavor.</p>
						</div>
					</div>
					<div class="col-lg-6">
						<div class="about-content-text pt-2">
							<p><i class="far fa-check-circle"></i> <span>Handyman Service</span></p>
						</div>
						<div class="about-content-text">
							<p><i class="far fa-check-circle"></i> <span>AC Maintenance Service</span></p>
						</div>
						<div class="about-content-text">
							<p><i class="far fa-check-circle"></i> <span>Plumbing Service</span></p>
						</div>
						<div class="about-content-text">
							<p><i class="far fa-check-circle"></i> <span>Carpenter Services</span></p>
						</div>
						<div class="about-content-text">
							<p><i class="far fa-check-circle"></i> <span>Electrical Service</span></p>
						</div>
						<div class="dreamit-button">
							<a href="#">About More </a>
						</div>
					</div>
					<!-- <div class="col-lg-6">
						<div class="about-content-text pt-2">
							<p><i class="far fa-check-circle"></i> <span>Repair Service</span></p>
						</div>
						<div class="about-content-text">
							<p><i class="far fa-check-circle"></i> <span>Plumbing Service</span></p>
						</div>
						<div class="about-content-text">
							<p><i class="far fa-check-circle"></i> <span>Handyman Services</span></p>
						</div>
						<div class="about-content-text">
							<p><i class="far fa-check-circle"></i> <span>Emergency Service</span></p>
						</div>
					</div> -->
				</div>
			</div>
		</div>
	</div>
</div>
<!--==================================================-->
<!-- End dreamit about Area -->
<!--==================================================-->

<!--==================================================-->
<!-- Start dreamit process Area -->
<!--==================================================-->
<div class="why-choose-us-area style-two pb-100">
	<div class="container-fluid">
		<div class="row align-items-center">	
			<div class="col-lg-6 col-md-6">
				<!-- <div class="slider-video">
					<div class="video-icon">
						<a class="video-vemo-icon venobox vbox-item" data-vbtype="youtube" data-autoplay="true" href="https://youtu.be/BS4TUd7FJSg"><i class="far fa-play-circle"></i></a>
					</div>
				</div>                                                                                                         -->
				<div class="dreamit-section-title white text-left">
					<h5>WHAT WE OFFER YOU</h5>
					<h2>Experience In The Electricals</h2>
					<h2>Maintenance <span>Industry</span></h2>
					<div class="em_bar_bg text-left"></div>
					<p>Elevating Spaces with Expert Electrical Maintenance. Fast, Reliable Service, Qualified Team, Affordable Pricing, and a Five-Year Warranty—Crafting Excellence for Your Peace of Mind.</p>
				</div>
				<div class="row">
					<div class="col-lg-6">
						<div class="why-choose-us-single-box">
							<div class="why-choose-us-icon">
								<i class="flaticon-electrician-3"></i>
							</div>
							<div class="why-choose-us-content">
								<div class="why-choose-us-title">
									<h2> Fast Service</h2>
									<p>Swift solutions for your electrical needs.</p>
								</div>
							</div>
						</div>
						<div class="why-choose-us-single-box">
							<div class="why-choose-us-icon">
								<i class="flaticon-electrician-5"></i>
							</div>
							<div class="why-choose-us-content">
								<div class="why-choose-us-title">
									<h2>Qualified Team</h2>
									<p>Skilled professionals at your service.</p>
								</div>
							</div>
						</div>
					</div>
					<div class="col-lg-6 col-md-12">
						<div class="why-choose-us-single-box">
							<div class="why-choose-us-icon">
								<i class="flaticon-electric-generator-1"></i>
							</div>
							<div class="why-choose-us-content">
								<div class="why-choose-us-title">
									<h2>Affordable Pricing</h2>
									<p>Transparent and budget-friendly.</p>
								</div>
							</div>
						</div>
						<div class="why-choose-us-single-box">
							<div class="why-choose-us-icon">
								<i class="flaticon-electrician-3"></i>
							</div>
							<div class="why-choose-us-content">
								<div class="why-choose-us-title">
									<h2>5 Months Warranty</h2>
									<p>Confidence in our lasting quality.</p>
								</div>
							</div>
						</div>
					</div>
				</div>
			</div>
			<div class="col-lg-6 col-md-12">
				<div class="why-choose-us-thumb1">
					<img src="assets/images/resource/wch1.jpg" alt="">
				</div>
			</div>
		</div>
	</div>
</div>
<!--==================================================-->
<!-- End dreamit process Area -->
<!--==================================================-->

<!--==================================================-->
<!-- Start dreamit about Area -->
<!--==================================================-->
<!-- <div class="about-area pt-85">
	<div class="container">
		<div class="row">	
			<div class="col-lg-6 col-md-6 col-sm-12">
				<div class="row">
					<div class="col-lg-12">
						<div class="dreamit-section-title text-left">
							<h5>Company</h5>
							<h2>1-2 years Experience In The</h2>
							<h2>Team  <span>Handyman</span></h2>
							<div class="em_bar_bg text-left"></div>
							<p>Proin gravida nibh vel velit auctor aliquet. sollicitudi lorem quis bibendum auctor, nisi elit consequat ipsum, sagittis sem nibh elit veniam, quis nostrud exercitation</p>
						</div>
						<div class="section-title-text">
							<p>There are many variations of passages of Lorem Ipsum a available, but the majority have suffered alteration in some form, by injected humour, or randomised words which don’t look even slightly this believable. If you are going to use a passage of Lorem Ipsum,</p>
						</div>
						<div class="stat-levels">
					<h6>Handyman Experience</h6>
				  <div class="stat-1 stat-bar">
				  	 <div class="stat-bar-rating" style="width: 90%;"><span class="percent">90%</span></div>
				  </div>
				  <h6>Plumbing Experience</h6>
				  <div class="stat-1 stat-bar">
				  	 <div class="stat-bar-rating" style="width: 80%;"><span class="percent">80%</span></div>
				  </div>
				  <h6>Industrie Experience</h6>
				  <div class="stat-1 stat-bar">
				  	 <div class="stat-bar-rating" style="width: 70%;"><span class="percent">70%</span></div>
				  </div>
				</div>
					</div>
				</div>
			</div>
			<div class="col-lg-6 col-md-6 col-sm-12">
				<div class="dreamit-about-thumb3">
					<img src="assets/images/project/team.png" alt="">
				</div>
			</div>
		</div>
	</div>
</div> -->
<!--==================================================-->
<!-- End dreamit about Area -->
<!--==================================================-->

<!--==================================================-->
<!-- Start dreamit team Area -->
<!--==================================================-->
<div class="team-area style-five pt-80 pb-65">
	<div class="container">
		<div class="row">
			<div class="col-lg-12 col-sm-12">
				<div class="dreamit-section-title pb-25 text-center">
					<h5>BEST SPECIALISTS</h5>
					<h2>Our Best Employees</h2>
					<h2>Team <span>Member</span></h2>
					<div class="em_bar_bg text-left"></div>
				</div>
			</div>	
		</div>
		<div class="row">
			<div class="col-lg-3 col-md-6 col-sm-12">
				<div class="dreamit-single-team-box">
					<div class="dreamit-single-team-thumb">
						<a href="#"><img src="assets/images/team/member1.png" alt=""></a>
					</div>
					<div class="team-content">
						<div class="team-title">
							<h3><a href="#">Hossen Babu Orfe</a></h3>
							<p>PLUMBER</p>
						</div>
						<div class="dreamit-social-icons">
							<div class="socail-icon">
								<a href="#"><i class="fab fa-facebook-f"></i></a>
								<a href="#"><i class="fab fa-twitter"></i></a>
								<a href="#"><i class="fab fa-linkedin-in"></i></a>
							</div>
						</div>
					</div>
				</div>
			</div>
			<div class="col-lg-3 col-md-6 col-sm-12">
				<div class="dreamit-single-team-box">
					<div class="dreamit-single-team-thumb">
						<a href="#"><img src="assets/images/team/member2.png" alt=""></a>
					</div>
					<div class="team-content">
						<div class="team-title">
							<h3><a href="#">Mostofa Kamal</a></h3>
							<p>AC MAINTENANCE</p>
						</div>
						<div class="dreamit-social-icons">
							<div class="socail-icon">
								<a href="#"><i class="fab fa-facebook-f"></i></a>
								<a href="#"><i class="fab fa-twitter"></i></a>
								<a href="#"><i class="fab fa-linkedin-in"></i></a>
							</div>
						</div>
					</div>
				</div>
			</div>
			<div class="col-lg-3 col-md-6 col-sm-12">
				<div class="dreamit-single-team-box">
					<div class="dreamit-single-team-thumb">
						<a href="#"><img src="assets/images/team/member3.png" alt=""></a>
					</div>
					<div class="team-content">
						<div class="team-title">
							<h3><a href="#">Mehedi Hassan Raj</a></h3>
							<p>ELECTRICAL</p>
						</div>
						<div class="dreamit-social-icons">
							<div class="socail-icon">
								<a href="#"><i class="fab fa-facebook-f"></i></a>
								<a href="#"><i class="fab fa-twitter"></i></a>
								<a href="#"><i class="fab fa-linkedin-in"></i></a>
							</div>
						</div>
					</div>
				</div>
			</div>
			<div class="col-lg-3 col-md-6 col-sm-12">
				<div class="dreamit-single-team-box">
					<div class="dreamit-single-team-thumb">
						<a href="#"><img src="assets/images/team/member2.png" alt=""></a>
					</div>
					<div class="team-content">
						<div class="team-title">
							<h3><a href="#">Mostofa Kamal</a></h3>
							<p>CARPENTER</p>
						</div>
						<div class="dreamit-social-icons">
							<div class="socail-icon">
								<a href="#"><i class="fab fa-facebook-f"></i></a>
								<a href="#"><i class="fab fa-twitter"></i></a>
								<a href="#"><i class="fab fa-linkedin-in"></i></a>
							</div>
						</div>
					</div>
				</div>
			</div>
		</div>
	</div>
</div>
<!--==================================================-->
<!-- End dreamit team Area -->
<!--==================================================-->

<!--==================================================-->
<!-- Start dreamit subscribe Area -->
<!--==================================================-->
<!--<div class="subscribe-area">
	<div class="container">
		<div class="row subscribe-color">
			<div class="col-lg-6 col-md-6 col-sm-12">
				<div class="subscribe-content">
					<div class="subscribe-title">
						<h2>Get Instant Access To</h2>
						<h4>Money Saving Coupons</h4>
					</div>
				</div>
			</div>
			<div class="col-lg-6 col-md-6 col-sm-12">
				<div class="contact-form-box">	
					<form id="contact_form" action="mail.php" method="POST">
						<div class="from-box1">
							<input type="text" placeholder="Enter Your Email*">
							<button>Subscribe</button>
						</div>
					</form>
					<p class="form-message"></p>
				</div>
			</div>
		</div>
	</div>
</div>-->
<!--==================================================-->
<!-- End dreamit subscribe Area -->
<!--==================================================-->

<!--==================================================-->
<!-- Start dreamit about Area -->
<!--==================================================-->
<div class="brand-area pt-80 pb-95">
	<div class="container">
		<div class="row">
			<div class="brand_list owl-carousel">
				<div class="col-lg-12">
					<div class="dreamit-single-brand">
						<div class="brand-thumb">
							<img src="assets/images/shape/brand1.png" alt="">
						</div>
					</div>
				</div>
				<div class="col-lg-12">
					<div class="dreamit-single-brand">
						<div class="brand-thumb">
							<img src="assets/images/shape/brand2.png" alt="">
						</div>
					</div>
				</div>
				<div class="col-lg-12">
					<div class="dreamit-single-brand">
						<div class="brand-thumb">
							<img src="assets/images/shape/brand3.png" alt="">
						</div>
					</div>
				</div>
				<div class="col-lg-12">
					<div class="dreamit-single-brand">
						<div class="brand-thumb">
							<img src="assets/images/shape/brand4.png" alt="">
						</div>
					</div>
				</div>
				<div class="col-lg-12">
					<div class="dreamit-single-brand">
						<div class="brand-thumb">
							<img src="assets/images/shape/brand5.png" alt="">
						</div>
					</div>
				</div>
			</div>
		</div>
	</div>
</div>
<!--==================================================-->
<!-- Start dreamit about Area -->
<!--==================================================-->

<?php include(APPPATH . 'Views/includes/footer.php'); ?>
    